...
export default function CoursesFeatures() {
  ...
  const womenCourses = [
    ...
    },
    {
      ...
      description: "تکمیل چهار شنا و شروع آموزش مهارت‌های مسابقه‌ای",
      schedule: [
        { days: "فرد", time: "۱۴:۰۰ - ۱۶:۰۰", pool: "استخر سجاد" }
      ]
    },
    {
      id: 107,
    ...
    },
    {
      ...
      type: "hydro",
      schedule: [
        { days: "فرد", time: "۱۰:۰۰ - ۱۱:۳۰", pool: "استخر کوثر" }
      ]
    }
  ];
  ...
  const currentCourses = useMemo(() => {
    switch(activeTab) {
      ...
      case "hydro":
        return [...hydroMenCourses, ...womenCourses.filter(course => course.type === "hydro")];
      default:
        return menCourses;
    }
  }, [activeTab]);
  ...
  const getPoolColor = (poolName) => {
    switch(poolName) {
      case 'استخر سجاد': return 'bg-blue-100 text-blue-700 border-blue-200';
      case 'استخر کوثر': return 'bg-emerald-100 text-emerald-700 border-emerald-200';
      ...
      <div className="w-full max-w-7xl">
        ...
        >
          <header className="text-center mt-12 sm:mt-4 mb-6 sm:mb-8 md:mb-12">
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-medium text-white mb-3 sm:mb-4">دوره‌های آموزشی ما</h1>
        ...
        {(activeTab === "men" || activeTab === "women") && progressPathCourses.length > 0 && (
          ...
          >
            <h3 className="text-white text-sm sm:text-base md:text-lg font-semibold text-center mb-3 sm:mb-4 md:mb-6">مسیر یادگیری شنای حرفه‌ای</h3>
            ...
            >
              ...
              <div className="space-y-2 sm:space-y-3 flex-grow flex flex-col">
                <div className="flex-grow">
                  <h3 className="font-semibold text-sky-900 text-xs sm:text-sm leading-tight mb-1.5 sm:mb-2 line-clamp-2">{course.title}</h3>
                  <p className="text-sky-700 text-[11px] sm:text-xs leading-relaxed mb-2 sm:mb-3 line-clamp-2">{course.description}</p>

                  <div className="mb-2 sm:mb-3">
                    <div className="text-[11px] sm:text-xs text-sky-600 font-medium mb-1.5 sm:mb-2">📅 برنامه کلاس‌ها:</div>
                    <div className="space-y-1">
                      {course.schedule.slice(0, 2).map((schedule, idx) => (
                        <div key={idx} className="flex flex-col sm:flex-row justify-between items-start sm:items-center text-[10px] sm:text-xs bg-sky-50 rounded-lg px-2 sm:px-3 py-1.5 sm:py-2 border border-sky-100 gap-1 sm:gap-0">
                          <span className="truncate">روزهای {schedule.days}</span>
                          <div className="flex gap-1 sm:gap-2 items-center">
                            <span className={`text-[9px] sm:text-[10px] px-1.5 sm:px-2 py-0.5 rounded-full border ${getPoolColor(schedule.pool)}`}>
                              {schedule.pool}
                            </span>
                            <span className="font-bold text-sky-700 whitespace-nowrap">{schedule.time}</span>
                          </div>
                        </div>
                      ))}
          ...
          {selectedCourse && (
            ...
            >
              ...
              >
                ...
                <h2 className="text-lg sm:text-xl font-bold text-sky-900 mb-1.5 sm:mb-2">{selectedCourse.title}</h2>
                <p className="text-sky-700 text-xs sm:text-sm mb-3 sm:mb-4 leading-relaxed">{selectedCourse.description}</p>

                <h4 className="font-bold text-sky-800 mb-1.5 sm:mb-2 text-xs sm:text-sm">📅 برنامه زمان‌بندی:</h4>
                <div className="space-y-1.5 sm:space-y-2 mb-4 sm:mb-6">
                  {selectedCourse.schedule.map((schedule, idx) => (
                    <div key={idx} className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-sky-50 rounded-lg px-3 sm:px-4 py-2 sm:py-2.5 border border-sky-100 gap-1 sm:gap-2">
                      <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                        <span className="font-medium text-xs sm:text-sm">روزهای {schedule.days}</span>
                        <span className={`text-[10px] sm:text-xs px-2 sm:px-2.5 py-0.5 rounded-full border ${getPoolColor(schedule.pool)}`}>
                          {schedule.pool}
                        </span>
                      </div>
                      <span className="bg-white px-2 sm:px-3 py-0.5 sm:py-1 rounded-lg text-sky-700 font-semibold text-[11px] sm:text-sm">{schedule.time}</span>
                    </div>
                  ))}
                </div>
                  ...
                  >
                    {enrollLoading === selectedCourse.id ? (
                      <>
                        ...
                        />
                        <span className="text-[11px] sm:text-xs">در حال ثبت‌نام...</span>
                      </>
                    ) : (
                      'ثبت‌نام در این دوره'
                    )}
