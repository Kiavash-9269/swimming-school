                ...
                <div className="aspect-[4/5] sm:aspect-[3/4] bg-gray-100 rounded-lg sm:rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-500">
                  ...
                  {category.id === "coaches" && image.alt && (
                    <div className="absolute bottom-3 left-3 right-3 p-2 sm:p-2.5 translate-y-0 md:translate-y-full md:group-hover:translate-y-0 transition-transform duration-300 bg-sky-500/90 backdrop-blur-sm rounded-lg shadow-lg">
                      <p className="text-white text-[11px] sm:text-xs font-medium text-center md:text-right">{image.alt}</p>
                    </div>
                  )}
                </div>
              </div>
  ...
  return (
    <div className="min-h-screen bg-sky-950 select-none">
      ...
      <div className="flex justify-center px-4 mb-10 sm:mb-14">
        <div className="flex overflow-x-auto gap-1 sm:gap-1.5 bg-white rounded-2xl p-1.5 shadow-inner max-w-full scrollbar-hide" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          <button
        ...
        {filtered.map((category, categoryIndex) => (
          ...
          >
            <div className="text-center space-y-2 sm:space-y-3">
              <h2 className="text-xl sm:text-2xl font-medium text-white">{category.title}</h2>
              <p className="text-gray-300 text-xs sm:text-sm max-w-xl mx-auto px-4">{category.description}</p>
            </div>
      ...
      >
        <div className="max-w-md mx-auto text-center px-4 space-y-3 sm:space-y-4">
          <h3 className="text-lg sm:text-xl font-medium text-white">آماده شروع هستید؟</h3>
          <p className="text-gray-300 text-xs sm:text-sm">همین امروز به خانواده ما بپیوندید</p>
