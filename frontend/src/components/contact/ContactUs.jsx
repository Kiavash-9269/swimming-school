...
//Components
const ContactCard = memo(({ icon, title, details, link }) => (
  <motion.div
    initial={{ opacity: 0, y: 10 }}
    animate={{ opacity: 1, y: 0 }}
    whileHover={{ y: -2 }}
    className="bg-white rounded-2xl p-4 border border-sky-200 hover:border-sky-300 transition-all duration-300 shadow-sm hover:shadow-md"
  >
    ...
    {link && (
      <a
        ...
        className="text-sky-600 hover:text-sky-800 text-xs mt-2 inline-block transition-colors duration-200"
      >
        ارتباط مستقیم →
      </a>
    )}
...
const SocialButton = memo(({ icon, name, link }) => (
  <motion.a
    ...
    target="_blank"
    rel="noreferrer noopener"
    className="w-10 h-10 flex items-center justify-center rounded-xl bg-sky-100 text-sky-700 border border-sky-200 hover:bg-sky-200 hover:text-sky-900 transition-all duration-200 shadow-sm"
    aria-label={name}
  >
    {icon}
  </motion.a>
...
const FAQItem = memo(({ item, index, isOpen, onToggle }) => (
  <div className="bg-white border border-sky-200 rounded-2xl shadow-sm overflow-hidden">
    <button
      ...
      className="w-full flex justify-between items-center p-4 text-sky-900 hover:bg-sky-50 transition-colors duration-200 text-sm font-medium"
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="p-4 text-sky-700 text-xs border-t border-sky-100"
        >
          {item.a}
        </motion.div>
      )}
...
const MapSection = memo(() => (
  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl shadow-sm border border-sky-200 p-4">
    ...
    <div className="h-48 rounded-xl overflow-hidden bg-sky-100 border border-sky-200">
      <iframe
        title="Location"
        src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d803.6532840000618!2d59.545977754768!3d36.32168724330435!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3f6c9213c3c8a575%3A0xb16f3f211b9dffe8!2z2KfYs9iq2K7YsSDYs9is2KfYrw!5e0!3m2!1sen!2s!4v1762856992257!5m2!1sen!2s"
        width="100%"
        height="100%"
        style={{ border: 0 }}
        loading="lazy"
...
const FAQSection = memo(({ openFAQ, toggleFAQ }) => (
  <section className="mt-12 mb-12 max-w-4xl mx-auto space-y-3">
            ...
            <form onSubmit={handleSubmit} className="space-y-4" noValidate>
              <div className="grid sm:grid-cols-2 gap-4">
                <input
                  ...
                  required
                  className="w-full px-4 py-3 rounded-xl border border-sky-200 bg-white focus:border-sky-400 focus:ring-2 focus:ring-sky-100 outline-none text-sky-800 text-sm transition-all duration-200 hover:shadow-sm"
                />
                <input
                  type="email"
                  name="email"
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-sky-200 bg-white focus:border-sky-400 focus:ring-2 focus:ring-sky-100 outline-none text-sky-800 text-sm transition-all duration-200 hover:shadow-sm text-right"
                />
                <select
                  name="subject"
              ...
              <textarea
                ...
                rows="4"
                required
                className="w-full px-4 py-3 rounded-xl border border-sky-200 bg-white focus:border-sky-400 focus:ring-2 focus:ring-sky-100 outline-none text-sky-800 text-sm transition-all duration-200 hover:shadow-sm resize-none"
              />
              ...
              <motion.button
                ...
                className="w-full bg-sky-600 text-white py-3 rounded-xl font-bold text-sm hover:bg-sky-700 transition-all duration-200 disabled:opacity-70 disabled:cursor-not-allowed shadow-sm hover:shadow-md"
              >
                {uiState.isSubmitting ? (
                  ...
                  <span className="flex items-center justify-center gap-2">
                    ارسال پیام <Send className="w-4 h-4" />
                  </span>
                )}
              </motion.button>
            </form>
