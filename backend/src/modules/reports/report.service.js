...
const { SHEET_NAMES, FILE_BASENAMES, localizeExportRow } = require("./exportLabels");
const {
  ENROLLMENT_STATUSES,
  PAYMENT_STATUSES,
  CLASS_STATUSES,
...
const EXPORT_COLUMNS = {
  participants: [
    { key: "id", header: "شناسه شرکت‌کننده", width: 26 },
    { key: "ownerUserId", header: "شناسه مالک حساب", width: 26 },
    { key: "firstName", header: "نام", width: 16 },
    { key: "lastName", header: "نام خانوادگی", width: 16 },
    { key: "birthDate", header: "تاریخ تولد", width: 14 },
    { key: "age", header: "سن", width: 8 },
    { key: "gender", header: "جنسیت", width: 10 },
    { key: "relation", header: "نسبت", width: 10 },
    { key: "phone", header: "تلفن", width: 14 },
    { key: "isActive", header: "فعال", width: 10 },
    { key: "hasEmergencyContact", header: "مخاطب اضطراری", width: 16 },
    { key: "insuranceStatus", header: "وضعیت بیمه", width: 16 },
    { key: "medicalStatus", header: "وضعیت پزشکی", width: 16 },
    { key: "enrollmentCount", header: "تعداد ثبت‌نام", width: 12 },
    { key: "createdAt", header: "تاریخ ایجاد", width: 22 },
  ],
  enrollments: [
    { key: "id", header: "شناسه ثبت‌نام", width: 26 },
    { key: "participantName", header: "شرکت‌کننده", width: 20 },
    { key: "ownerUserId", header: "شناسه مالک حساب", width: 26 },
    { key: "classTitle", header: "کلاس", width: 24 },
    { key: "instructorName", header: "مربی", width: 18 },
    { key: "status", header: "وضعیت", width: 18 },
    { key: "basePrice", header: "قیمت پایه (ریال)", width: 16 },
    { key: "discountAmount", header: "تخفیف (ریال)", width: 14 },
    { key: "finalAmount", header: "مبلغ نهایی (ریال)", width: 16 },
    { key: "priceCharged", header: "مبلغ دریافت‌شده (ریال)", width: 16 },
    { key: "eligibilityEligible", header: "واجد شرایط هنگام ثبت", width: 16 },
    { key: "createdAt", header: "زمان ثبت‌نام", width: 22 },
    { key: "confirmedAt", header: "زمان تأیید", width: 22 },
    { key: "cancelledAt", header: "زمان لغو", width: 22 },
  ],
  payments: [
    { key: "id", header: "شناسه پرداخت", width: 26 },
    { key: "userId", header: "شناسه کاربر", width: 26 },
    { key: "enrollmentId", header: "شناسه ثبت‌نام", width: 26 },
    { key: "classId", header: "شناسه کلاس", width: 26 },
    { key: "amount", header: "مبلغ (ریال)", width: 14 },
    { key: "status", header: "وضعیت", width: 16 },
    { key: "provider", header: "درگاه", width: 12 },
    { key: "providerRef", header: "کد پیگیری درگاه", width: 20 },
    { key: "createdAt", header: "زمان ایجاد", width: 22 },
    { key: "verifiedAt", header: "زمان تأیید", width: 22 },
  ],
  attendance: [
    { key: "id", header: "شناسه حضور", width: 26 },
    { key: "classId", header: "شناسه کلاس", width: 26 },
    { key: "sessionId", header: "شناسه جلسه", width: 26 },
    { key: "participantName", header: "شرکت‌کننده", width: 20 },
    { key: "status", header: "وضعیت", width: 12 },
    { key: "markedAt", header: "زمان ثبت", width: 22 },
  ],
  classes: [
    { key: "id", header: "شناسه کلاس", width: 26 },
    { key: "title", header: "عنوان", width: 28 },
    { key: "instructorName", header: "مربی", width: 18 },
    { key: "startDate", header: "تاریخ شروع", width: 14 },
    { key: "endDate", header: "تاریخ پایان", width: 14 },
    { key: "totalSessions", header: "تعداد جلسات", width: 12 },
    { key: "price", header: "قیمت (ریال)", width: 14 },
    { key: "capacity", header: "ظرفیت", width: 10 },
    { key: "confirmedCount", header: "تأییدشده", width: 12 },
    { key: "heldCount", header: "رزرو موقت", width: 12 },
    { key: "remainingCapacity", header: "ظرفیت باقی‌مانده", width: 14 },
    { key: "waitlistCount", header: "لیست انتظار", width: 12 },
    { key: "status", header: "وضعیت", width: 18 },
  ],
  waitlist: [
    { key: "id", header: "شناسه لیست انتظار", width: 26 },
    { key: "classTitle", header: "کلاس", width: 24 },
    { key: "participantName", header: "شرکت‌کننده", width: 20 },
    { key: "position", header: "رتبه", width: 10 },
    { key: "status", header: "وضعیت", width: 12 },
    { key: "notifiedAt", header: "زمان اطلاع‌رسانی", width: 22 },
    { key: "expiresAt", header: "انقضا", width: 22 },
    { key: "createdAt", header: "زمان ایجاد", width: 22 },
  ],
};
...
async function exportReport(reportType, query, { adminUserId }) {
  ...
  try {
    ...
    const rows = (data.items || []).map((row) => localizeExportRow(reportType, row));
    const buffer = await buildWorkbookBuffer({
      sheetName: SHEET_NAMES[reportType] || "گزارش",
      columns,
      rows,
      rightToLeft: true,
    });

    const dateStamp = new Date().toLocaleDateString("fa-IR").replace(/\//g, "-");
    const base = FILE_BASENAMES[reportType] || "گزارش";
    const filename = sanitizeFilename(`${base}-${dateStamp}.xlsx`);
